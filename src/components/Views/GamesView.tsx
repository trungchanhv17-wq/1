...
import { ArticleQuiz } from '../Vocabulary/ArticleQuiz';
...
const LEVEL_CONFIG = {
  ...
  B1: {
    title: "Intermediate (Trung cấp B1)",
          ...
          <div className="space-y-2 pt-1">
            <div className="flex justify-between items-end">
              <div className="flex items-center gap-2">
                ...
                <span className={cn("text-[9px] font-black uppercase tracking-[.15em]", config.text)}>Arena Progress</span>
              </div>
...
export function GamesView() {
  const [viewMode, setViewMode] = useState<'LEVELS' | 'CURRICULUM' | 'ARTICLE_QUIZ' | 'AIC_LESSON'>('CURRICULUM');
  ...
  useEffect(() => {
    ...
    const allMeanings = Array.from(new Set(VOCABULARY_DATA.map(w => w.meaning)))
    ...
    setQuizSelectedOption(null);
    setQuizChecked(false);
  }, [quizIndex, selectedLevel, activeTheme, filteredWords]);
    ...
    return (
      <div className="flex flex-col h-full bg-[#F5F7FB] relative overflow-hidden">
        ...
        <div className="p-6 border-b border-border/50 bg-white/80 backdrop-blur-md flex items-center justify-between sticky top-0 z-50">
          ...
          >
            <ArrowLeft className="w-4 h-4" /> Thoát Đấu Trường
          </button>
          <div className="flex flex-col items-center">
             <div className="text-[10px] font-bold text-text-secondary uppercase tracking-[0.2em] mb-0.5">Game Arena</div>
             <div className="text-sm font-black text-slate-900">Grammar Articles Drilling</div>
          </div>
          <div className="w-24" />
        </div>
  ...
  if (viewMode === 'CURRICULUM') {
    ...
    return (
      <div className="flex h-[calc(100vh-120px)] bg-slate-50 -m-6 md:-m-12 overflow-hidden border-t border-slate-200">
        {/* Sidebar */}
        <div className={cn(
          ...
          <div className="p-6 pb-2">
            <div className="flex items-center gap-4 mb-6">
              <div className={cn("w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black text-2xl shadow-lg relative overflow-hidden", config.bg)}>
                <span className="relative z-10 italic">{selectedLevel}</span>
              </div>
              <div>
                <h2 className="text-xl font-black tracking-tighter text-slate-900 leading-tight">Game Arena</h2>
            ...
            {/* Level Selector Tabs */}
            <div className="mb-6">
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Cấp độ thử thách</div>
                ...
                {(['A1', 'A2', 'B1', 'B2'] as Level[]).map((lvl) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "py-2 rounded-lg text-xs font-black transition-all cursor-pointer text-center",
                        isSelected 
                          ? cn("text-white shadow-sm", lvlConfig.bg)
          ...
          <div className="flex-grow overflow-y-auto px-4 pb-6 space-y-1">
            ...
            <div className="h-4" />
            <p className="px-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-2">Chủ đề bài học</p>
        ...
        <div className={cn(
          ...
          <div className="p-4 bg-white border-b-2 border-slate-200">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div className="md:hidden">
                ...
                >
                  <ArrowLeft className="w-4 h-4" /> Quay lại Chủ đề
                </button>
              </div>

              {/* Sub-tabs system exactly from image */}
          ...
          <div className="flex-grow overflow-y-auto p-4 md:p-8">
            {activeTab === 'LEARN' && (
              <>
                <div className="hidden md:block bg-white rounded-[2rem] border-2 border-slate-200 shadow-xl shadow-slate-200/20 overflow-hidden mb-8">
                    ...
                    <tbody className="divide-y divide-slate-50">
                      {filteredWords.map((word) => (
                        <tr key={word.id} className="group hover:bg-slate-50/80 transition-colors">
                          <td className="px-8 py-5">
                            <div className="flex items-center gap-3">
                              <button 
                                ...
                                className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-primary hover:text-white transition-all scale-100"
                              >
                          ...
                          <td className="px-6 py-5">
                            <p className="text-sm font-bold text-slate-600">{word.meaning}</p>
                          </td>
                  ...
                  {filteredWords.map((word) => (
                    <div key={word.id} className="bg-white rounded-2xl border-2 border-slate-200 p-5 shadow-sm active:bg-slate-50 transition-colors flex flex-col gap-3">
                      ...
                      <div className="border-t border-slate-100 pt-3">
                        ...
                        <p className="text-sm font-bold text-slate-800">{word.meaning}</p>
                      </div>

                      {word.example && (
                        <div className="bg-slate-50 rounded-xl p-3 border border-slate-100">
                          <span className="text-[9px] font-black uppercase tracking-widest text-slate-400 block mb-1">Example</span>
                          <p className="text-xs text-slate-600 font-medium italic leading-relaxed">{word.example}</p>
                        </div>
                      )}
                    </div>
                  ))}
                          ...
                          <div className="text-xs text-slate-400 text-center italic mt-2">
                            {genderChecked ? (
                              <div className="text-slate-600 font-medium">
                                Nghĩa tiếng Việt: <strong className="text-slate-900">{currentWord.meaning}</strong>
                              </div>
                            ) : (
                            ...
                            {fillBlankChecked && (
                              ...
                              >
                                <div className="flex items-center gap-2 font-black text-sm">
                                  {fillBlankIsCorrect ? "Tuyệt vời! Đáp án chính xác" : "Cố lên! Đáp án chưa chính xác"}
                                </div>
                                <div className="text-xs font-semibold">
                                  Đáp án đúng: <span className="underline font-mono font-bold text-slate-900 text-sm">{currentWord.article ? `${currentWord.article} ` : ''}{currentWord.word}</span> {currentWord.plural ? `(Plural: ${currentWord.plural})` : ''}
                            ...
                            {quizChecked ? (
                              <div className="space-y-3">
                                {quizSelectedOption === -1 && (
                                  <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-center text-xs font-black uppercase text-rose-600 animate-pulse">
                              ...
                              <div className="h-14 flex items-center justify-center text-xs text-slate-400 italic">
                                Hãy chọn một đáp án phía trên để kiểm tra kết quả!
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="p-8 md:p-12 flex flex-col justify-between bg-slate-50/50">
                          <div>
                            ...
                            <div className="my-6 bg-white border border-slate-200/80 rounded-2xl p-8 shadow-sm flex flex-col items-center justify-center min-h-[160px]">
                              ...
                              <span className="text-xs text-slate-400 font-bold mt-4 animate-pulse">
                        ...
                        <div className="p-8 md:p-12 flex flex-col justify-between space-y-8 bg-white">
                          <div className="space-y-4">
                            ...
                            {listenChecked && (
                              ...
                              >
                                <div className="flex items-center gap-2 font-black text-sm">
                                  {listenIsCorrect ? "Rất tốt! Bạn đã nghe chính xác" : "Cố lên! Hãy xem kết quả"}
                                </div>
                                <div className="text-xs font-semibold">
                                  Đáp án đúng: <span className="underline font-mono font-bold text-slate-900 text-sm">{currentWord.article ? `${currentWord.article} ` : ''}{currentWord.word}</span>
                                </div>
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 md:p-10 space-y-12 animate-in fade-in duration-500">
      <SectionHeader 
        title="Game Arena" 
        subtitle="Hone your vocabulary, grammar, and listening reflex through gamified training." 
        icon={Zap}
      />
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-xl font-black tracking-tight text-slate-900">Chọn cấp độ thử thách</h2>
          </div>
          <div className="text-xs text-text-secondary font-bold uppercase tracking-widest bg-slate-100 px-3 py-1 rounded-full text-slate-500">
            4 Cấp độ khả dụng
          </div>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
             ...
             <h2 className="text-xl font-black tracking-tight text-slate-900">Grammar Drills & Quizzes</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
           <motion.div 
             ...
             className="bg-white border text-[#130a68] border-slate-200 rounded-[2.5rem] p-8 cursor-pointer group hover:border-primary/20 transition-all shadow-xl overflow-hidden relative"
           >
